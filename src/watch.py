...
class Watcher():
    ...
    def check_time(self, config: ConfigElement) -> bool:
        current_time = datetime.now().strftime('%H:%M').time()
    ...
    def check_running(self, programs: list[str]) -> list:
        tested_positive = []
        all_running_programs = [process.Name.lower() for process in self.wmi.Win32_Process()]
        for program in programs:
            if str(program.lower() + ".exe") in all_running_programs:
